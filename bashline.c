...
#if defined (HAVE_LLM)
static int llm_cycle_server (int, int);
#endif
...
#if defined (HAVE_LLM)
/*
 * Shift-Tab handler: cycle to next LLM server.
 * Uses the same globals from llm.def (g_servers, g_ctx, etc.)
 */
static int
llm_cycle_server (int count, int key)
{
  ...
  if (!g_llm_initialized || !g_servers || g_servers->count < 2)
    {
      rl_ding ();
      return 0;
    }
  ...
  llm_api_cleanup ();
  llm_api_init (s->api_url, s->model, s->api_key);
  ...
  /* Show which server is now active */
  rl_crlf ();
...
#if defined (HAVE_LLM)
  /* Shift-Tab cycles through configured LLM servers */
  rl_bind_keyseq ("\033[Z", llm_cycle_server);
#endif
