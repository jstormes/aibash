                                  ...
                                  const char *result)
{
    ...
    char *truncated = NULL;
    if (strlen(result) >= BASH_LLM_MAX_OUTPUT_CAPTURE) {
        truncated = malloc(BASH_LLM_MAX_OUTPUT_CAPTURE);
        if (truncated) {
            memcpy(truncated, result, BASH_LLM_MAX_OUTPUT_CAPTURE - 20);
            strcpy(truncated + BASH_LLM_MAX_OUTPUT_CAPTURE - 20, "\n[truncated]");
            content = truncated;
        }
    }
